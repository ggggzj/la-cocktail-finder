...
function AppContent() {
  ...
  const handleToggleFavorite = (barId: string) => {
    ...
    const isAdding = !favoriteIds.includes(barId);
    ...
    if (bar) {
      showToast(
        isAdding 
          ? `❤️ Added "${bar.name}" to your favorites!`
          : `💔 Removed "${bar.name}" from favorites`,
          ...
          <div className="flex items-center justify-between h-24">
            <div className="flex items-center space-x-4">
              <div className="relative group">
                <div className="w-16 h-16 bg-gradient-to-r from-yellow-400 via-pink-500 to-purple-600 rounded-3xl flex items-center justify-center shadow-2xl transform rotate-12 group-hover:rotate-0 transition-all duration-500 hover:scale-110">
                ...
                <div className="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-blue-400 to-cyan-500 rounded-full animate-ping"></div>
              </div>
              <div>
                <h1 className="text-4xl font-black bg-gradient-to-r from-white via-yellow-200 to-pink-200 bg-clip-text text-transparent drop-shadow-lg animate-fade-in">
                  LA Cocktail Finder
                </h1>
                <p className="text-white/90 text-sm mt-1 font-medium animate-slide-in-right">🌟 Discover magical cocktail experiences in Los Angeles ✨</p>
              </div>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-6">
              {loading && (
                <div className="flex items-center space-x-3 text-white/90">
                  <LoadingSpinner size="sm" className="text-yellow-300" />
                  <span className="font-medium animate-pulse">Loading amazing bars...</span>
                </div>
              )}
